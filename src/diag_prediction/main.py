...
class diagnose():
    ...
    """
    This class is for training and testing the RNN models for diagnosis prediction
    ...
    def tf_train_health(self,lookahead, dense, dimension, num_classes, batch_size, hid_layer,  num_epochs, x_train, y_train, x_test, y_test):
        ...
        for step in range(num_epochs):    # training
            ...
            preds  = sess.run( output, {tf_x: x_test})
            print("Epoch multilabel accuracy", step, self.accuracy_multilabel(preds, y_test))
    ...
    def accuracy_multilabel(self,predict_test, y_test):
        ...
        # total is number of test samples
        for i in range(0,total):
            #print('dim_y_test=%d dim_predict_test=%d' % \
                    #(len(y_test[i]), len(predict_test[i])))
            k = max(10,len(set(y_test[i])))
            preds = predict_test[i]
            #get the 'k' elemenst predicted to have high score (or true)
            top_pred = preds.argsort()[-1*k:]
            #print("Predicted : " + str(set(top_pred)) + "\tActual :" +
            #      str(set(y_test[i])))
    ...
    def accuracy_freq20(self, predict_test, y_test):
        freq20_diag = list([89, 86, 96, 54, 94, 225, 52, 246, 1, 58, 119 144,
                            211, 48, 83, 84, 244, 126, 145, 110])
        ...
        for i in range(0, len(predict_test)):
            ...
            check5 = (set(freq20_diag)).intersection(set(y_test[i]))
            check20 = (set(freq20_diag)).intersection(set(y_test[i]))
    ...
    #TODO check implementation: Raghav
    # Take diagnosis codes mapped to 0-284 codes, considering least frequent dignosis-codes
    # with count > 100 (more than 100 visits) 
    ...
    def run_model(self, lookahead=3, dense=False, dimension=32,  num_classes=None, batch_size=64, hid_layer=64, num_epochs=20, x_train = None, y_train = None, x_test = None, y_test = None ):
        ...
        accuracy_rare20 = self.accuracy_rare20(preds, y_test)
        accuracy_freq20 = self.accuracy_freq20(preds, y_test)
        print("Test data accuracy : all_diagnosis, rare diagnosis(top5, top20), freq diagnosis(top5, top20), ", accuracy, accuracy_rare20, accuracy_freq20)


        #tensorflow + attention on 284 diagnosis codes ( valid only when lookahead = 2 )
        print('tf accuracy with attention')
        num_epochs_att = 50
        preds, att = self.tf_train_health_attention(lookahead, dense, dimension,  num_classes, batch_size, hid_layer, num_epochs_att, x_train, y_train, x_test, y_test)
        accuracy = self.accuracy_multilabel(preds, y_test)
        accuracy_rare20 = self.accuracy_rare20(preds, y_test)
        accuracy_freq20 = self.accuracy_freq20(preds, y_test)
        print("Test data accuracy (attention): all_diagnosis, rare diagnosis(top5, top20), freq diagnosis(top5, top20), ", accuracy, accuracy_rare20, accuracy_freq20)
...
def main(emb_file, dimension):
    #lookahead_array = [2]
    lookahead = 2
    #dimension_array = [32]
    dense = True  #False indicates multihot, True indicates dense
    d = diagnose(lookahead=lookahead, dimension=dimension, dense=dense, emb_file=emb_file)
...
if __name__ == "__main__":
    if(len(sys.argv) < 3):
        print("Usage <emb_file> <emb_dimension>")
    else:
        print("Running patient model with embeddings", sys.argv[1])
