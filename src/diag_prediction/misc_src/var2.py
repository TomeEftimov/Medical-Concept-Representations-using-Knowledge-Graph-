...
class Preprocess():
	...
	def sparsify(self, inv_name):

		l = []
		for i in range(0, len(self.co_G)):
			for j in range(0,len(self.co_G[i])):
				l.append(self.co_G[i][j])

		l = np.array(l)
		mean = np.mean(l)
		std = np.std(l)

		w = open('sparse_desc.txt','w')

		for i in range(0, len(self.co_G)):
			for j in range(0,len(self.co_G[i])):
				if self.co_G[i][j] >= mean+std:
					w.write(inv_name[i].replace(',',' ')+','+inv_name[j].replace(',',' ')+'\n')
	...
	def __init__(self):
		...
		finv = open('inv_name','rb')
		inv_name = pkl.load(finv)
		finv.close()
		...
		for name, group in td.groupby('SUBJECT_ID'):	
			...
			if len(l) >= 1:
				...
				# for i in range(0,len(hadm_list)-3):
				# 	self.data.append([hadm_code_dict[hadm_list[i]], hadm_code_dict[hadm_list[i+1]], hadm_code_dict[hadm_list[i+2]], hadm_code_dict[hadm_list[i+3]] ])
