...
class Preprocess():
	...
	def generate_onehot_enc(self,data):	
		...
		for pt in data:
			...
			for i in range(0,len(pt)):
				...
				seq.append(np.array(pt_exp))

			embed_data.append(np.array(seq))
	...
	def generate_encodings(self,dimension, dense):
		...
		train_seq = self.seq_length(np.array(train_d))
		...
		test_seq = self.seq_length(np.array(test_d))
		...
		return self.pad(x_train, max_seq), y_train, self.pad(x_test, max_seq), y_test, train_seq, test_seq
		...
		for x in d:
			...
			if len(x) < seq:
				for j in range(len(x), seq):
					pad_d = [0]*284
					x.append(np.array(pad_d))

			new_d.append(np.array(x))

		return np.array(new_d)

	def seq_length(self,d):
		slen = []
		...
		for name, group in td.groupby('SUBJECT_ID'):	
			...
			if len(l) >= 3 and len(l) <= 12:
				...
				for c in l:
					...
					hadm_code_dict[c[0]] = np.array(icd9_codes)

				hadm_list.sort(key=self.cmp_to_key(self.compare_datetime))


				for k in range(2, len(hadm_list)):
					ex = []
					for i in range(0,k+1):
						ex.append(hadm_code_dict[hadm_list[i]])

					self.data.append(np.array(ex))
				...
				# our whole data is just 4919 examples!!
