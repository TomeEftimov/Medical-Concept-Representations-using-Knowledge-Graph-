...
from single_level import inv_name
...
def tf_train_health(dense, dimension, batch_size, hid_layer,  num_epochs, x_train, y_train, x_test, y_test):
...
def find_error(predict_test, y_test):
...
def names(l):
	return [inv_name[t] for t in l]

def code_args(l):
	return [i for i in range(0,len(l)) if l[i] == 1]
...
x_train, y_train, x_test, y_test = pre.generate_data(dim, dense)
preds = tf_train_health(dense, dim, batch_size, hid_layer, num_epochs, x_train, y_train, x_test, y_test)
print(find_error(preds, y_test))
