...
class Preprocess():
	...
	def generate_data(self,dimension, dense):
		...
		train_size = int(0.80*len(self.data))
	...
	def inc_isa(self, data, isa, rare):
		upd_data = []
		
		for i in range(0,len(data)):	
			upd_data.append(data[i])
			if data[i] in rare :
				for j in range(0, len(isa[data[i]])):
					upd_data.append(isa[data[i]][j])

		return list(set(upd_data))


	def addEdge(self,icd):

		for i in range(0,len(icd)):
			for j in range(i+1, len(icd)):
				self.edList = self.edList | set([(icd[i],icd[j])]) | set([(icd[j],icd[i])]) 
	...
	def __init__(self):
		...
		with open('isA.rel','rb') as f_isa:
			isa = pkl.load(f_isa)

		with open('diag_count.pkl','rb') as fdi:
			rare = pkl.load(fdi)
			# rare = rare[0:175]
		...
		for name, group in td.groupby('SUBJECT_ID'):	
			...
			if len(l) >= lookahead:
				...
				# for i in range(0,len(hadm_list)-2):
				# 	self.data.append([hadm_code_dict[hadm_list[i]], hadm_code_dict[hadm_list[i+1]], hadm_code_dict[hadm_list[i+2]]])



				# for i in range(0,len(hadm_list)-2):
				# 	#isA enhanced version
				# 	t = self.inc_isa(hadm_code_dict[hadm_list[i]], isa, rare)
				# 	t_1 = self.inc_isa(hadm_code_dict[hadm_list[i+1]], isa, rare)
				# 	t_2 = (hadm_code_dict[hadm_list[i+2]])
				# 	self.data.append([t,t_1,t_2])
